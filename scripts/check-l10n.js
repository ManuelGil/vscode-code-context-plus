...
function main() {
  ...
  if (hasMissing || hasUnused) {
    ...
    if (hasUnused) {
      console.error('\nResult: Unused (orphan) i18n keys found. Remove them from bundles or delete dead l10n.t calls.');
    }
    process.exit(1);
  } else {
    console.log('\nResult: All used i18n keys are present in bundles, with no orphan keys.');
  }
}
